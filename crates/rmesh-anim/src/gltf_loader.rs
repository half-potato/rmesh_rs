...
fn image_to_rgba8(data: &gltf::image::Data) -> Vec<u8> {
    match data.format {
        gltf::image::Format::R8 => data.pixels.iter().flat_map(|&r| [r, r, r, 255]).collect(),
...
pub fn load_gltf(path: &Path) -> Result<GltfScene> {
    ...
    for mesh in document.meshes() {
        for (prim_idx, primitive) in mesh.primitives().enumerate() {
            ...
            let normals_raw: Option<Vec<[f32; 3]>> =
                reader.read_normals().map(|iter| iter.collect());
            let uvs_raw: Option<Vec<[f32; 2]>> = reader
                .read_tex_coords(0)
                .map(|iter| iter.into_f32().collect());
            let tangents_raw: Option<Vec<[f32; 4]>> =
            ...
            let (flat_pos, flat_nor, flat_uv, flat_tan) = if let Some(ref idx) = indices {
                ...
                for &i in idx {
                    ...
                    if let Some(ref n) = normals_raw {
                        nor.push(n[ii]);
                    }
                    if let Some(ref u) = uvs_raw {
                        uv.push(u[ii]);
                    }
                    if let Some(ref t) = tangents_raw {
                        tan.push(t[ii]);
                    }
                }
            ...
            let tangents: Vec<[f32; 4]> = flat_tan
                .iter()
                .map(|t| {
    ...
    for root_node in gltf_scene.nodes() {
        queue.push_back(QueueEntry {
            gltf_node_index: root_node.index(),
            parent_idx: coord_root_idx,
        });
    }

    while let Some(entry) = queue.pop_front() {
        let gltf_node = document
            .nodes()
            .nth(entry.gltf_node_index)
        ...
        for channel in anim.channels() {
            ...
            let values: Vec<f32> = match reader.read_outputs() {
                Some(outputs) => match outputs {
                    ...
                    gltf::animation::util::ReadOutputs::Scales(iter) => iter.flatten().collect(),
    ...
    scene.playback = PlaybackState {
        clip_index: if scene.clips.is_empty() {
            None
        } else {
            Some(0)
        },
