// Interval generate: per-tet compute shader that decomposes each visible tet
// into screen-space triangles for the tiled interval rasterizer.
//
// Runs after project_compute. Indexed by vis_idx from compact_tet_ids.
// Outputs pixel-space silhouette vertices + per-tet material data.
//
// Adapted from interval_compute.wgsl.
...
@group(0) @binding(6) var<storage, read> compact_tet_ids: array<u32>;
@group(0) @binding(7) var<storage, read> indirect_args: DrawIndirectArgs;
@group(0) @binding(8) var<storage, read_write> interval_verts: array<vec4<f32>>;
@group(0) @binding(9) var<storage, read_write> interval_tet_data: array<vec4<f32>>;
@group(0) @binding(10) var<storage, read_write> interval_meta: array<u32>;
@group(0) @binding(11) var<storage, read> vertex_normals: array<f32>;
...
// Write a vertex at the given slot.
// Layout: [slot*4+0] = (pixel_x, pixel_y, z_front_ndc, z_back_ndc)
//         [slot*4+1] = (off_front, off_back, 0, 0)
//         [slot*4+2] = (n_front.xyz, 0)
//         [slot*4+3] = (n_back.xyz, 0)
fn write_vertex(slot: u32, pixel_xy: vec2<f32>, z_front: f32, z_back: f32,
                off_front: f32, off_back: f32, n_front: vec3<f32>, n_back: vec3<f32>) {
    interval_verts[slot * 4u + 0u] = vec4<f32>(pixel_xy, z_front, z_back);
    interval_verts[slot * 4u + 1u] = vec4<f32>(off_front, off_back, 0.0, 0.0);
    interval_verts[slot * 4u + 2u] = vec4<f32>(n_front, 0.0);
    interval_verts[slot * 4u + 3u] = vec4<f32>(n_back, 0.0);
}

// Write a degenerate vertex (zero position, zero data).
fn write_degenerate_vertex(slot: u32) {
    interval_verts[slot * 4u + 0u] = vec4<f32>(0.0);
    interval_verts[slot * 4u + 1u] = vec4<f32>(0.0);
    interval_verts[slot * 4u + 2u] = vec4<f32>(0.0);
    interval_verts[slot * 4u + 3u] = vec4<f32>(0.0);
}
...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    let v_base = vis_idx * 5u;  // 5 vertices per tet
    ...
    // Perspective divide -> NDC and pixel coords
    ...
    for (var vi = 0u; vi < 4u; vi++) {
        ...
        // NDC to global pixel coords
        pixel_xy[vi] = vec2<f32>(
            (ndc_xy[vi].x + 1.0) * 0.5 * W,
            (1.0 - ndc_xy[vi].y) * 0.5 * H,
        );
    }
    ...
    interval_tet_data[vis_idx] = vec4<f32>(base_color, tet_density);
    ...
    // Classify silhouette (using pixel coords — same topology as NDC)
    ...
    if interior_vtx < 4u {
        ...
        let bary = bary2d(pixel_xy[center], pixel_xy[si0], pixel_xy[si1], pixel_xy[si2]);
        ...
        // Normals for center vertex
        let n_face = bary.x * vn[si0] + bary.y * vn[si1] + bary.z * vn[si2];
        let n_center = vn[center];

        var offset_front: f32;
        var offset_back: f32;
        var center_nf: vec3<f32>;
        var center_nb: vec3<f32>;
        if z_face <= z_center {
        ...
        // Slots 0-2: silhouette vertices (pixel coords)
        // At silhouette verts, n_front = n_back = vn[vi] (bary weight 1.0)
        for (var i = 0u; i < 3u; i++) {
            write_vertex(v_base + u32(i), pixel_xy[sv_idx[i]],
                         ndc_z[sv_idx[i]], ndc_z[sv_idx[i]],
                         sv_offset[i], sv_offset[i],
                         vn[sv_idx[i]], vn[sv_idx[i]]);
        }
        // Slot 3: copy of vert 0 (wraps fan, makes tri 3 degenerate)
        write_vertex(v_base + 3u, pixel_xy[sv_idx[0]],
                     ndc_z[sv_idx[0]], ndc_z[sv_idx[0]],
                     sv_offset[0], sv_offset[0],
                     vn[sv_idx[0]], vn[sv_idx[0]]);
        // Slot 4: center vertex
        write_vertex(v_base + 4u, pixel_xy[center], z_front, z_back,
                     offset_front, offset_back, center_nf, center_nb);

        // Meta: 3 silhouette verts, packed with tet_id
        interval_meta[vis_idx] = (3u) | (tet_id << 4u);
        ...
        let center_pix = mix(pixel_xy[ea.x], pixel_xy[ea.y], ta);
        ...
        // Normals for center vertex via edge interpolation
        let n_ea = mix(vn[ea.x], vn[ea.y], ta);
        let n_eb = mix(vn[eb.x], vn[eb.y], tb);
        ...
        for (var i = 0u; i < 4u; i++) {
            write_vertex(v_base + u32(i), pixel_xy[sv[i]],
        ...
        write_vertex(v_base + 4u, center_pix, z_front, z_back,
                     offset_front, offset_back, center_nf, center_nb);

        // Meta: 4 silhouette verts, packed with tet_id
        interval_meta[vis_idx] = (4u) | (tet_id << 4u);
    }
}
