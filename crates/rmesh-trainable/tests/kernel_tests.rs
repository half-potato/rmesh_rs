...
use rmesh_train::{
    create_adam_bind_group, create_loss_bind_group, record_adam_pass, record_loss_pass,
...
use rmesh_util::test_util::{
    build_test_scene, compute_circumspheres, create_rw_buffer, create_test_device,
    random_single_tet_scene, read_buffer, TestDeviceConfig,
};
...
fn test_radix_sort_kernel() {
    ...
    let radix_pipelines =
        rmesh_trainable::RadixSortPipelines::new(&device, 1, rmesh_trainable::SortBackend::Drs);
    let radix_state =
        rmesh_trainable::RadixSortState::new(&device, n, 32, 1, rmesh_trainable::SortBackend::Drs);
...
fn test_loss_compute_kernel() {
    ...
    let loss_bg = create_loss_bind_group(&device, &loss_pipeline, &loss_buffers, &rendered_buf);
    ...
    assert!(dl_d[3].abs() < tol, "dl_d_a[0] = {}, expected 0", dl_d[3]);
...
fn test_tile_fill_kernel() {
    ...
    let tet_count = 4u32;
    let tile_size = 4u32;
    let tile_buffers = rmesh_trainable::TileBuffers::new(&device, tet_count, W, H, tile_size);
    let tile_pipelines = rmesh_trainable::TilePipelines::new(&device);

    // Write tile uniforms
    let tile_uni = rmesh_trainable::TileUniforms {
        screen_width: W,
        screen_height: H,
        tile_size,
    ...
    let test_keys = create_rw_buffer(
        &device,
        "test_keys",
        (tile_buffers.max_pairs_pow2 as u64) * 8,
    );
    let test_values = create_rw_buffer(
        &device,
        "test_values",
        (tile_buffers.max_pairs_pow2 as u64) * 4,
    );
    ...
    let keys: Vec<u32> = read_buffer(
        &device,
        &queue,
        &test_keys,
        (tile_buffers.max_pairs_pow2 * 2) as usize,
    );
    let values: Vec<u32> = read_buffer(
        &device,
        &queue,
        &test_values,
        tile_buffers.max_pairs_pow2 as usize,
    );
    ...
    for (i, &k) in keys.iter().enumerate() {
        assert_eq!(
            k, 0xFFFFFFFF,
            "Key word at {i} = {k:#x}, expected 0xFFFFFFFF"
        );
    }
    ...
    eprintln!(
        "tile_fill: verified {} pairs ({} u32s)",
        tile_buffers.max_pairs_pow2,
        tile_buffers.max_pairs_pow2 * 2
    );
}
...
fn test_multi_tet_gradient_finite_diff() {
    ...
    let run_forward_loss = |scene_data: &SceneData, base_colors: &[f32]| -> f32 {
        ...
        let radix_pipelines =
            rmesh_trainable::RadixSortPipelines::new(&device, 2, rmesh_trainable::SortBackend::Drs);
        let tile_buffers =
            rmesh_trainable::TileBuffers::new(&device, scene_data.tet_count, W, H, tile_size);
        let sorting_bits = rmesh_trainable::sorting_bits_for_tiles(
            tile_buffers.num_tiles,
            rmesh_trainable::SortBackend::Drs,
        );
        ...
        let scan_pipelines = rmesh_trainable::ScanPipelines::new(&device);
        let scan_buffers = rmesh_trainable::ScanBuffers::new(&device, scene_data.tet_count);
        ...
        let rendered: Vec<f32> =
            read_buffer(&device, &queue, &rasterize.rendered_image, n_pixels * 4);
    ...
    // -----------------------------------------------------------------------
    let run_backward = |scene_data: &SceneData,
                        base_colors: &[f32]|
     -> (Vec<f32>, Vec<f32>, Vec<f32>, Vec<f32>) {
        ...
        // Backward
        let grad_buffers = rmesh_trainable::GradientBuffers::new(
            &device,
            scene_data.vertex_count,
            scene_data.tet_count,
        );
        let mat_grad_buffers =
            rmesh_trainable::MaterialGradBuffers::new(&device, scene_data.tet_count);
        ...
        let bwd_tiled_pipelines = rmesh_trainable::BackwardTiledPipelines::new(&device);
        let (bwd_bg0, bwd_bg1) = rmesh_trainable::create_backward_tiled_bind_groups(
            &device,
            &bwd_tiled_pipelines,
            &buffers.uniforms,
        ...
        let mut encoder = device.create_command_encoder(&wgpu::CommandEncoderDescriptor::default());
        {
            ...
            pass.dispatch_workgroups(num_tiles.min(65535), num_tiles.div_ceil(65535).max(1), 1);
        }
        queue.submit(std::iter::once(encoder.finish()));

        let d_densities: Vec<f32> = read_buffer(
            &device,
            &queue,
            &grad_buffers.d_densities,
            scene_data.tet_count as usize,
        );
        let d_vertices: Vec<f32> = read_buffer(
            &device,
            &queue,
            &grad_buffers.d_vertices,
            (scene_data.vertex_count * 3) as usize,
        );
        let d_color_grads: Vec<f32> = read_buffer(
            &device,
            &queue,
            &mat_grad_buffers.d_color_grads,
            (scene_data.tet_count * 3) as usize,
        );
        let d_base_colors: Vec<f32> = read_buffer(
            &device,
            &queue,
            &mat_grad_buffers.d_base_colors,
            (scene_data.tet_count * 3) as usize,
    ...
    // -----------------------------------------------------------------------
    let (d_densities, d_vertices, d_color_grads, d_base_colors) =
        run_backward(&scene, &base_colors_init);
    ...
    let check_grad_eps =
        |name: &str, analytical: f32, param_plus_loss: f32, param_minus_loss: f32, epsilon: f32| {
    ...
    for i in 0..scene.densities.len() {
        ...
        let (rel_err, abs_err, analytical, numerical) = check_grad_eps(
            &format!("density[{i}]"),
            d_densities[i],
            loss_plus,
            loss_minus,
        ...
        if rel_err > rel_tol
            && abs_err > abs_tol
            && (analytical.abs() > 1e-7 || numerical.abs() > 1e-7)
        {
...
fn test_single_tet_loss_decreases() {
    ...
    let adam_state = AdamState::new(&device, scene.vertex_count, scene.tet_count);
    let mat_adam = rmesh_train::MaterialAdamState::new(&device, scene.tet_count);
    let adam_pipeline = AdamPipeline::new(&device);
    ...
    for step in 1..=num_steps {
        ...
        for (&bg, &count) in adam_bgs.iter().zip(param_counts.iter()) {
            ...
            let wg_count: u32 = count.div_ceil(256);
            pass.dispatch_workgroups(wg_count.min(65535), wg_count.div_ceil(65535).max(1), 1);
        }
        ...
        let rendered: Vec<f32> =
            read_buffer(&device, &queue, &rasterize.rendered_image, n_pixels * 4);
        let cpu_loss: f32 = rendered
            .chunks(4)
            .zip(gt_data.chunks(3))
            .map(|(r, g)| {
                let dr = r[0] - g[0];
                let dg = r[1] - g[1];
                let db = r[2] - g[2];
        ...
        if step == 1 {
            ...
            let d_dens: Vec<f32> = read_buffer(
                &device,
                &queue,
                &grad_buffers.d_densities,
                scene.tet_count as usize,
            );
            let d_cg: Vec<f32> = read_buffer(
                &device,
                &queue,
                &mat_grad_buffers.d_color_grads,
                (scene.tet_count * 3) as usize,
            );
            let dens_vals: Vec<f32> = read_buffer(
                &device,
                &queue,
                &buffers.densities,
                scene.tet_count as usize,
    ...
    eprintln!(
        "Initial: {initial_loss:.8}, Final: {final_loss:.8}, Ratio: {:.4}",
...
fn test_camera_cpu_projection() {
    ...
    for vi in 0..4 {
        ...
        assert!(
            ndc.x >= -1.0 && ndc.x <= 1.0,
            "vertex {vi} ndc.x = {}, out of [-1,1]",
            ndc.x
        );
        assert!(
            ndc.y >= -1.0 && ndc.y <= 1.0,
            "vertex {vi} ndc.y = {}, out of [-1,1]",
            ndc.y
        );
        assert!(
            ndc.z >= 0.0 && ndc.z <= 1.0,
            "vertex {vi} ndc.z = {}, out of [0,1]",
            ndc.z
        );
...
fn test_camera_gpu_visibility() {
    ...
    assert!(tiles[0] > 0, "Expected tiles_touched > 0, got {}", tiles[0]);
}
...
fn test_camera_ray_tet_intersection() {
    ...
    assert!(
        t_exit > t_enter,
        "t_exit ({t_exit}) should be > t_enter ({t_enter})"
    );
    ...
    assert!(
        hits > 0,
        "At least some pixels near center should hit the tet"
    );
}
...
fn test_interval_tiled_gradient_finite_diff() {
    ...
    let run_forward_loss = |scene_data: &SceneData, base_colors: &[f32]| -> f32 {
        ...
        // Interval tiled buffers + pipelines
        let interval_buffers =
            rmesh_trainable::IntervalTiledBuffers::new(&device, scene_data.tet_count);
        let interval_gen = rmesh_trainable::IntervalGeneratePipeline::new(&device);
        let interval_gen_bg = rmesh_trainable::create_interval_generate_bind_group(
            &device,
            &interval_gen,
        ...
        let interval_rasterize =
            rmesh_trainable::IntervalTiledRasterizePipeline::new(&device, W, H, 0);
        let interval_rasterize_bg_a = rmesh_trainable::create_interval_tiled_rasterize_bind_group(
            &device,
            &interval_rasterize,
            &buffers.uniforms,
        ...
        let rendered: Vec<f32> = read_buffer(
            &device,
            &queue,
            &interval_rasterize.rendered_image,
            n_pixels * 4,
    ...
    let run_backward = |scene_data: &SceneData,
     ...
     -> (Vec<f32>, Vec<f32>, Vec<f32>, Vec<f32>) {
        ...
        let interval_grad_buffers = rmesh_trainable::IntervalGradBuffers::new(
            &device,
            scene_data.tet_count,
            scene_data.vertex_count,
            scene_data.tet_count,
        ...
        let bwd_interval_tiled = rmesh_trainable::BackwardIntervalTiledPipeline::new(&device, 0);
        let (bwd_it_bg0, bwd_it_bg1) = rmesh_trainable::create_backward_interval_tiled_bind_groups(
            &device,
            &bwd_interval_tiled,
            &buffers.uniforms,
            &loss_buffers.dl_d_image,
        ...
        // interval_chain_back
        let chain_back = rmesh_trainable::IntervalChainBackPipeline::new(&device);
        let (cb_bg0, cb_bg1) = rmesh_trainable::create_interval_chain_back_bind_groups(
            &device,
            &chain_back,
            &buffers.uniforms,
            &buffers.vertices,
        ...
        rmesh_trainable::record_backward_interval_tiled(
            &mut encoder,
            &bwd_interval_tiled,
            &bwd_it_bg0,
            &bwd_it_bg1,
        ...
        rmesh_trainable::record_interval_chain_back(
            &mut encoder,
            &chain_back,
            &cb_bg0,
            &cb_bg1,
    ...
    for i in 0..scene.vertices.len() {
        ...
        let (_rel_err, abs_err, analytical, numerical) = check_grad_eps(
            &format!("vertex[{i}]"),
            d_vertices[i],
            loss_plus,
            loss_minus,
    ...
    if any_failed {
        panic!(
            "Some interval tiled gradients failed the finite-difference check! See output above."
        );
    }
